...
##Respository address: https://github.com/morgansong/AutoCar.git
            ...
            try: 
                ...
                if '1' in str(str_share):
                    ...
                    if flag_back:
                        ...
                        self.PWM1.ChangeDutyCycle(pwmduty)
                        self.PWM2.ChangeDutyCycle(pwmduty)
                        self.PWM3.ChangeDutyCycle(pwmduty)
                        self.PWM4.ChangeDutyCycle(pwmduty)
                    ...
                    if flag_left:
                        print("trun left")
                        self.PWM1.ChangeDutyCycle(pwmduty_low)
                        self.PWM2.ChangeDutyCycle(pwmduty)
                        self.PWM3.ChangeDutyCycle(pwmduty_low)
                        self.PWM4.ChangeDutyCycle(pwmduty)

                    if flag_right:
                        print("trun right")
                        self.PWM1.ChangeDutyCycle(pwmduty)
                        self.PWM2.ChangeDutyCycle(pwmduty_low)
                        self.PWM3.ChangeDutyCycle(pwmduty)
                        self.PWM4.ChangeDutyCycle(pwmduty_low) 
                else:
                    for ENA in self.Pin_Setting: 
                        gpio.output(ENA[1][0], gpio.LOW)    #weel1-AIN1-LOW
                        gpio.output(ENA[1][1], gpio.LOW)   #weel1-AIN2-LOW
