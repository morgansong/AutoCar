...
public class RockerView extends View {
     ...
     * @return 摇杆实际显示的位置（点）
     */
    private Point getRockerPositionPoint(Point centerPoint, Point touchPoint, float regionRadius, float rockerRadius) throws InterruptedException {
        // 两点在X轴的距离
     ...
     * @param angle 摇动角度
     */
    private void callBack(double angle) throws InterruptedException {
        if (null != mOnAngleChangeListener) {
//            mOnAngleChangeListener.angle(angle);
            mOnAngleChangeListener.angle(angle, mRockerPosition.x, mCenterPoint.x, mRockerPosition.y,mCenterPoint.y);
        }
        if (null != mOnShakeListener) {
    ...
    public interface OnAngleChangeListener {
         ...
         */
        void angle(double angle, double x, double x1, double y,double y1) throws InterruptedException;
